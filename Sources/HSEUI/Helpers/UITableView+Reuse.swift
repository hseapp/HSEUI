...
extension UITableView {
    ...
    func register<T: UIView>(_ type: T.Type,
        ...
        switch kind {
        case .cell:
            register(BaseTableViewCell<T>.self, forCellReuseIdentifier: reuseId)
            
        case .headerFooter:
            register(BaseTableViewHeaderFooterCell<T>.self, forHeaderFooterViewReuseIdentifier: reuseId)
        }
    }
    
    func dequeue<T: UIView>(_ type: T.Type,
                            for indexPath: IndexPath,
                            kind: ElementKind = .cell,
                            reuseId: String) -> UIView {
        switch kind {
        case .cell:

...
public class TitleWithSubtitleViewModel: CellViewModel {
    ...
    public init(title: String,
        ...
        super.init(view: TitleWithSubtitleView.self, configureView: { view in
            view.update(title: title, text: text, attributedText: attributedText, numberOfTitleLines: numberOfTitleLines, allowSelection: allowSelection, tapCallback: tapCallback, useChevron: useChevron, pasteBoardText: pasteBoardText, pasteBoardCompletion: pasteBoardCompletion)
...
open class TitleWithSubtitleView: CellView {
    ...
    open override func commonInit() {
        addSubview(title)
        title.stickToSuperviewEdges([.left, .top, .right], insets: .init(top: 11, left: 16, bottom: 0, right: 16))

        addSubview(subtitle)
        subtitle.stickToSuperviewEdges([.left, .right, .bottom], insets: .init(top: 0, left: 16, bottom: 13, right: 16))
        subtitle.top(2, to: title)
